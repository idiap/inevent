...
def get_similar_events(event_id, limit=5):
    return get_inEvent_data("findSimilarHyperEvents?hyperEventId=" + str(event_id),
                            json.dumps({"firstResult": 0, "maxResults": limit}))
...
def parse_hyperevent(event, only_basic_info, include_recommendation=False):
    ...
    for key in event.iterkeys():
        ...
        if (key == 'description' and only_basic_info == False):
            if event['description'] is None or len(event['description']) != 0 and event[
                'description'].isspace() == False or event['description'] != "None":
                ...
                if (track['mimeType'] == "inevent/video"):
                    for file in files:
                        if (file['mimeType'].rfind('image') != -1):  # Get Snapshot
                            hyperevent['snapshot_url'] = rest_url_retrieval + "getTrackFile/" + file[
                                'fileName'] + "?trackId=" + str(file['trackId'])
                        #                            print hyperevent['snapshot_url']
                        elif (file['mimeType'].rfind('video') != -1):
                            hyperevent['video_url'] = rest_url_retrieval + "getTrackFile/" + file[
                                'fileName'] + "?trackId=" + str(file['trackId'])
                    ...
                    for file in files:
                        if (file['mimeType'].rfind('text/plain') != -1):  # Get transcript
                            hyperevent['transcript_url'] = rest_url_retrieval + "getTrackFile/" + file[
                                'fileName'] + "?trackId=" + str(file['trackId'])
                    ...
                    for file in files:
                        if file['fileName'] is None:
                            continue
                        url = rest_url_retrieval + "getTrackFile/" + file['fileName'] + "?trackId=" + str(
                            file['trackId'])
                    ...
                    if (len(files) > 0):
                        ...
                        for i in xrange(len(emotions['emotion_classes'])):
                            hyperevent['emotions'].update(
                                {emotions['emotion_classes'][i].lower(): emotions['emotion_scores'][i] * 100})
